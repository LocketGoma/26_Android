...
class GameRenderer(context: Context, private val input: InputController) : GLSurfaceView.Renderer {
    ...
    fun resume() { clock.reset(); paused = false }

    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        renderer.create()
        resources = ResourceManager(applicationContext)
        scenes = SceneManager(ShooterScene(ShootingSpriteCatalog.load(resources)))
